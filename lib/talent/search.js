...
function buildWhere(opts) {
    ...
    if (opts.country)
        where.country = { contains: opts.country, mode: "insensitive" };
    if (opts.funded !== undefined)
        where.funded = opts.funded;
    if (opts.source)
...
export function scorePosition(p, q) {
    ...
    if (nq) {
        if (fields.title.includes(nq))
            score += 60;
        else if (fields.institution.includes(nq))
            score += 45;
        else if (fields.topics.includes(nq))
            score += 30;
        else if (fields.description.includes(nq))
            score += 18;
    }
...
export async function searchPositions(opts = {}) {
    ...
    return candidates
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        .map((p) => ({ p, score: scorePosition(p, opts.q ?? "") }))
